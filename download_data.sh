...
# See README.md "Downloading data" for usage. Run with --help for a flag summary.
...
and extract into sibling folders of this script. Resumable on network drop.
...
Options:
  ...
  --with-unitmatch      Include the per-session UnitMatch/ subfolders when
                        extracting. Excluded by default to keep data/ lean —
                        UnitMatch/ holds inputs for cross-session unit matching
                        and is only needed for that specific workflow.
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --with-unitmatch) WANT_UNITMATCH=true; shift ;;
...
main() {
    ...
    if [[ "$WANT_UNITMATCH" == true ]]; then
        echo "  UnitMatch/:    included (--with-unitmatch)"
    else
        echo "  UnitMatch/:    skipped at extract (default; bytes still download)"
    fi
    ...
    local data_excludes=()
    if [[ "$WANT_LFP" == false ]]; then
        data_excludes+=("*/lfp.signal.npy" "*/lfp.times.npy" "*/lfp.metrics.htsv")
    fi
    if [[ "$WANT_UNITMATCH" == false ]]; then
        data_excludes+=("*/UnitMatch/*")
    fi

    if [[ ${#data_excludes[@]} -gt 0 ]]; then
        download_and_extract "data.zip" "$DATA_DIR" "${data_excludes[@]}"
    else
